...
# Uses incoming solar from US-NR1 (2008-2013), NOT subnivian lab data
...
# install.packages("REddyProc")
 install.packages("ncdf4") # ncdf package depricatded in R 3.5.3
  ...
  setwd(dir)
  file <- paste(dir,"Filled_makeNetCDF_1.00.R", sep = "")
  ...
  # only allow snow in May
#  temp <- Data.F$PRECTmms
#  temp[ which(Data.F$DoY >  120 & Data.F$TBOT <= 273) ] <- NA
#  temp[ which(Data.F$DoY <= 150 & Data.F$TBOT <= 273) ] <- NA 
#  temp
  
  
  Data.F$PRECTmms[Data.F$TBOT <= 273] <- Data.F$PRECTmms[Data.F$TBOT <= 273] * 1.00 
  min(Data.F$FLDS)
